# Offline Reinforcement Learning for Pipeline Diagnosis
This project proposes a **deep reinforcement learning framework** grounded in **sequential decision-making**, leveraging **DQN** and **D3QN** models for training and inference on one-dimensional time-series data. 
Experiments conducted on a pipeline leak dataset, obtained from simulated lab environments, demonstrate that the framework achieves strong classification performance under **low-data regimes** and can be extended to other **anomaly detection** tasks.
本项目提出了一种基于**序贯决策**的**深度强化学习框架**，并在此框架上使用**DQN模型**、**D3QN模型**于一维时序数据上完成训练、推理。  
经过本人在学校提供的模拟实验中获取的管道泄漏数据集上进行的实验，
可以证明该框架在**小样本条件**下有较强的分类性能，并可推广应用于其他**异常检测**任务。
***

## 🧠Methodology/方法




## 🔍Preparation/准备




## ⚙️Installation/环境配置





## 🚀Usage/使用方法





## 📁Project Structure/项目结构





## ⚠️Notes/注意事项





## 📚Citation/引用




## 📄License/许可证




